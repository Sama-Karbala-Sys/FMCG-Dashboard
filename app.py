...
st.markdown("""
    <style>
        ...
        div[data-testid="stExpander"] {
            ...
            border-radius: 12px; border: 1px solid #334155; transition: all 0.3s ease;
        }
...
def load_slh_general_data():
    ...
    try:
        ...
        # البحث الذكي عن أسماء الأعمدة المعقدة
...
try:
    ...
    if st.session_state.active_tab == 'slh_gen':
        ...
        if not df_slh_gen.empty:
            ...
            with st.expander("🔍 فلاتر قسم عام المجزر", expanded=True):
                with st.form("slh_gen_form"):
                    f1, f2 = st.columns(2)
                    sel_cat = f1.multiselect("🏷️ التصنيف", filtered_slh_gen[c_cat].unique() if c_cat and c_cat in filtered_slh_gen.columns else [])
                    sel_item = f2.multiselect("📦 المادة", filtered_slh_gen[c_item].unique() if c_item and c_item in filtered_slh_gen.columns else [])
                    submitted_slh_gen = st.form_submit_button("🚀 تطبيق الفلاتر")
            ...
            k4.metric("🚨 مواد حرجة (تكفي < 7 أيام)", str(critical_items), delta="- خطر نفاد المخزون", delta_color="inverse")
            st.markdown("---")
            ...
            try:
                ...
                with row1_c1:
                    if c_cat and c_bal and c_cat in filtered_slh_gen.columns and c_bal in filtered_slh_gen.columns:
                        fig_cat = px.pie(filtered_slh_gen.groupby(c_cat)[c_bal].sum().reset_index(), values=c_bal, names=c_cat, hole=0.4, title="🏷️ توزيع الرصيد حسب التصنيف", color_discrete_sequence=px.colors.sequential.Agsunset)
                ...
                with row1_c2:
                    if c_item and c_coverage and c_item in filtered_slh_gen.columns and c_coverage in filtered_slh_gen.columns:
                        ...
                        fig_crit = px.bar(crit_df, x=c_coverage, y=c_item, orientation='h', color=c_coverage, color_continuous_scale='Reds_r', text_auto='.1f', title="⚠️ المواد الأكثر حرجاً (أيام كفاية الرصيد)")
                        fig_crit.update_layout(showlegend=False, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font=dict(color='#cbd5e1'), xaxis_title="عدد الأيام")
            ...
            try:
                if c_item and c_bal and c_confirmed and c_item in filtered_slh_gen.columns:
                    ...
                    fig_comp = px.bar(melted, x=c_item, y='الكمية', color='النوع', barmode='group', title="⚖️ مقارنة: الرصيد الفعلي مقابل المثبت (لأعلى 15 مادة)", color_discrete_map={c_bal: '#3b82f6', c_confirmed: '#10b981'}, text_auto='.2s')
                    fig_comp.update_layout(legend_title_text='', xaxis_title="", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font=dict(color='#cbd5e1'))
            ...
            with st.form("custom_slh_gen_form"):
                ...
                x_axis = ca1.selectbox("اختر حقل المقارنة:", cat_cols) if cat_cols else None
                y_axis = ca2.selectbox("اختر القيمة المراد حسابها:", num_cols) if num_cols else None
            ...
            st.markdown("---")
            with st.expander("📋 عرض جدول عام المجزر الشامل"):
                csv_slh_gen = filtered_slh_gen.to_csv(index=False).encode('utf-8')
                st.download_button(label="📥 تحميل التقرير (CSV)", data=csv_slh_gen, file_name="slh_general_data.csv", mime="text/csv")
                st.dataframe(filtered_slh_gen, use_container_width=True)
        else:
            st.warning("⚠️ جاري سحب البيانات، يرجى الانتظار والمحاولة مرة أخرى.")
